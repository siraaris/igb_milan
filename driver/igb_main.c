...
char igb_driver_name[] = "igb_milan";
...
static bool disable_vlan_filter;
module_param(disable_vlan_filter, bool, 0644);
MODULE_PARM_DESC(disable_vlan_filter, "Disable hardware VLAN filtering in RX path");
...
{
	/* Experimental: mimic igb_avb normal-LAN queue selection. */
	return 3;
}
...
static void igb_set_rx_mode(struct net_device *netdev)
{
	...
	u32 rctl, vmolr = 0, rlpml = MAX_JUMBO_FRAME_SIZE;
	int count;
				   ...
				   struct sk_buff *skb)
{
	...
	if (gptp_dbg_count < 64 && skb->protocol == htons(ETH_P_1588) &&
	    skb->len >= 1) {
		struct ethhdr *eth = eth_hdr(skb);
		u8 msg_type = skb->data[0] & 0x0f;

		netdev_info(dev,
			    "gptp dbg rx[%d]: src=%pM dst=%pM msg=%u len=%u q=%u ts=%u tsip=%u vlan=%u vid=%u\n",
			    gptp_dbg_count,
			    eth->h_source, eth->h_dest,
			    msg_type, skb->len, rx_ring->queue_index,
			    igb_test_staterr(rx_desc, E1000_RXDADV_STAT_TS) ? 1 : 0,
			    igb_test_staterr(rx_desc, E1000_RXDADV_STAT_TSIP) ? 1 : 0,
			    skb_vlan_tag_present(skb) ? 1 : 0,
			    skb_vlan_tag_present(skb) ? skb_vlan_tag_get(skb) : 0);
		gptp_dbg_count++;
	}
}
